...
void addEdge(Graph* g, const char* src, const char* dest, int weight) {
    ...
    if(pair == NULL) return;
    List * adjacencyLista = (List *) pair->value;
    Edge* newEdge = (Edge *)malloc(sizeof(Edge));
    if(newEdge == NULL)return;
    newEdge->weight = weight;
    newEdge->target = strdup(dest);
    list_pushBack(adjacencyLista,newEdge);
...
int getWeight(Graph* g, const char* label1, const char* label2) {
    ...
    return -1; // Si no existe el origen o terminamos de iterar sin encontrar el destino
}
...
List* getAdjacentLabels(Graph* g, const char* label) {
    ...
    for(Edge * edge = list_first(edges); edge != NULL; edge = list_next(edges)){
